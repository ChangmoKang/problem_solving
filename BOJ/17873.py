import sys
sys.stdin = open('input/17873.txt')
    ...
    for time in range(1, 1001):
        for idx in range(K):
            ...
            if not (r == rr and c == cc):
                if board[rr][cc] == 1:
                    dic[rr, cc].extend(dic[r, c][start:][::-1])
                else:
                    dic[rr, cc].extend(dic[r, c][start:])
...
dic = {(i, j): [] for i in range(N) for j in range(N)}
